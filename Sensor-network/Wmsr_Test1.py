...
class LCP(Network):
    ...
    def consensus_alg(self, v, iter_time, f):
        ...
        if iter_time % 10 == 0 and iter_time > 12:
            ...
            index = 0
            index_end = len(neighbor_values) - 1
            cur_value = self.G.node[v]['value'][iter_time-1]
            while index < f:
                if neighbor_values[index_end] > cur_value:
                    index_end -= 1
                    index += 1
                    ...
                    break
            if neighbor_values[index_end+1:]:
                x = min(neighbor_values[index_end+1:])
            else:
                x = float("inf")
            for j in neighbors:
                if self.G.node[j]['value'][iter_time-1] < x:
