...
class WACS_alg(Network_alg):
    ...
    def standard_consensus_alg(self, iter_time):
        ...
        for i in range(1,11):
            for j in range(1,11):
                if abs(self.G.nodes[i]['value'][iter_time]-self.G.nodes[j]['value'][iter_time]) > self.G.nodes[i]['lan'][iter_time]:
                    if self.adjMatrix[i-1,j-1] == 1:
                        self.adjMatrix[i-1,j-1] = 0
                        self.DeMatrix[i-1,i-1] = self.DeMatrix[i-1, i-1]-1
                        print abs(self.G.nodes[i]['value'][iter_time]-self.G.nodes[j]['value'][iter_time])
                        print self.G.nodes[i]['lan'][iter_time]
        ...
        self.VaMatrix = np.dot(I - 1.0/5* L, self.VaMatrix)
        self.VaMatrix[2,0] = 12
